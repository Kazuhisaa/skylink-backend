...
async def create_payment_intent(
    ...
    intent_data = await paymongo_service.create_payment_intent(
        amount_centavos, description, metadata
    )
    ...
    return {"client_key": client_key, "payment_intent_id": intent_id}
...
async def _process_successful_payment(gateway_ref: str):
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(Payment)
            .where(Payment.gateway_ref == gateway_ref)
        ...
        if payment and payment.status != "paid":
            ...
            await db.commit()
            logger.info(
                f"Payment {gateway_ref} marked as PAID. Booking {payment.booking_id} confirmed."
            )
...
def verify_webhook_signature(payload_bytes: bytes, signature: str, timestamp: str):
    ...
    expected_sig = hmac.new(
        settings.PAYMONGO_WEBHOOK_SECRET.encode(), to_sign, hashlib.sha256
    ).hexdigest()
