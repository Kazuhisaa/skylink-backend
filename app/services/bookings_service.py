...
from app.schemas.bookings import BookingCreate, CancelRequest, RescheduleRequest
...
async def _get_booking_with_relations(booking_id: uuid.UUID, db: AsyncSession) -> Booking:
    result = await db.execute(
        ...
        .options(
            ...
            .selectinload(Flight.seat_pricing)
            .selectinload(FlightSeatPricing.seat_class),
...
async def _get_pricing(
    flight_id: uuid.UUID, seat_class_id: int, db: AsyncSession
) -> FlightSeatPricing:
    result = await db.execute(
...
async def get_user_bookings(
    user_id: uuid.UUID, db: AsyncSession, page: int = 1, size: int = 10
