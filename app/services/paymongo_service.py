...
class PayMongoService:
    ...
    async def create_payment_intent(
        self, amount: int, description: str, metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"PayMongo error: {e.response.json().get('errors', 'Unknown error')}",
                )
            except Exception as e:
                logger.error(f"Unexpected error calling PayMongo: {str(e)}")
                raise HTTPException(
                    status_code=500, detail="Internal server error calling payment gateway"
                )
