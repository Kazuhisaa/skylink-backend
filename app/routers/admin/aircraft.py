...
router = APIRouter(prefix="/admin", tags=["Admin - Aircraft"])
...
@router.post(
    "/aircraft", response_model=AircraftRead, status_code=201, dependencies=[Depends(require_admin)]
)
@limiter.limit("30/minute")
...
@router.put(
    "/aircraft/{aircraft_id}", response_model=AircraftRead, dependencies=[Depends(require_admin)]
)
@limiter.limit("30/minute")
async def edit_aircraft(
    request: Request, aircraft_id: int, body: AircraftUpdate, db: AsyncSession = Depends(get_db)
):
...
@router.get(
    "/seat-classes", response_model=list[SeatClassRead], dependencies=[Depends(require_admin)]
)
@limiter.limit("30/minute")
...
@limiter.limit("30/minute")
async def add_seat_class(
    request: Request, body: SeatClassCreate, db: AsyncSession = Depends(get_db)
):
    return await aircraft_service.create_seat_class(body, db)


@router.put(
    "/seat-classes/{seat_class_id}",
    response_model=SeatClassRead,
    dependencies=[Depends(require_admin)],
)
@limiter.limit("30/minute")
async def edit_seat_class(
    request: Request, seat_class_id: int, body: SeatClassUpdate, db: AsyncSession = Depends(get_db)
):
    return await aircraft_service.update_seat_class(seat_class_id, body, db)


@router.delete(
    "/seat-classes/{seat_class_id}", status_code=204, dependencies=[Depends(require_admin)]
)
@limiter.limit("30/minute")
async def remove_seat_class(
    request: Request, seat_class_id: int, db: AsyncSession = Depends(get_db)
):
...
@router.get(
    "/aircraft/{aircraft_id}/seats",
    response_model=list[AircraftSeatRead],
    dependencies=[Depends(require_admin)],
)
@limiter.limit("30/minute")
async def list_aircraft_seats(
    request: Request, aircraft_id: int, db: AsyncSession = Depends(get_db)
):
...
@limiter.limit("30/minute")
async def add_aircraft_seats(
    request: Request,
    aircraft_id: int,
    body: list[AircraftSeatCreate],
