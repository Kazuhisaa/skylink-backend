...
@router.post(
    "/airports", response_model=AirportRead, status_code=201, dependencies=[Depends(require_admin)]
)
@limiter.limit("30/minute")
...
@router.put(
    "/airports/{airport_id}", response_model=AirportRead, dependencies=[Depends(require_admin)]
)
@limiter.limit("30/minute")
async def edit_airport(
    request: Request, airport_id: int, body: AirportUpdate, db: AsyncSession = Depends(get_db)
):
